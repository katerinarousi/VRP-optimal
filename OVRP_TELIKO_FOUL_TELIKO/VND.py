...
class VND:
    ...
    def solve(self):
        self.AddDepotAtTheEndOfEachRoute() #in order to handle my OVRP with algorithms constructed for CVRP, with vehicles that return in the depot.
        self.VND()
        self.RemoveLastDepot(self.sol) #return to my Open VRP Solution. Cost is also consistent as the distance from any node to depot is set to 0.0
        self.ReportSolution(self.sol)
        SolDrawer.draw('final_vnd', self.sol, self.allNodes)
            ...
            for originNodeIndex in range(1, len(rt1.sequenceOfNodes) - 1):
                for targetRouteIndex in range (0, len(self.sol.routes)):
                    ...
                    for targetNodeIndex in range (0, len(rt2.sequenceOfNodes) - 1):
                        ...
                        rt1copy = self.cloneRoute(rt1)  #I apply the relocation to rt1 and rt2's clones and moveCost is being calculated 
                        rt2copy = self.cloneRoute(rt2)  #by the difference in cost between the copies and the original routes.
