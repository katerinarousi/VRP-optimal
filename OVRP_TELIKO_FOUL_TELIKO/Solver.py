...
class Clarke_n_Wright:
    ...
    def UpdateRouteCostAndLoad(self, rt: Route):
        ...
        tl = self.empty_vehicle_weight + td #total load in the beginning of each route
        tn_km = 0
        for i in range(len(rt.sequenceOfNodes) - 1):
            A = rt.sequenceOfNodes[i]
            B = rt.sequenceOfNodes[i+1]
            tn_km += self.distanceMatrix[A.ID][B.ID] * tl
            tl -= B.demand #The demand of the just visited customer is being removed from the load and the vehicle continues to the next customer.
    ...
    def calculate_savings(self):
        ...
        for i in range(len(self.customers)):
            ...
            for j in range(i + 1, len(self.customers)): #anw trigwnikos pinakas
                ...
                score = (self.empty_vehicle_weight + n1.demand) * self.distanceMatrix[self.depot.ID][n1.ID]
    ...
    def Clarke_n_Wright(self):
        ...
        for i in range(len(savings)): 
            ...
            if n1.route == n2.route: #belong to the same route
                continue
            if self.not_first_or_last(rt1, n1) or self.not_first_or_last(rt2, n2): #not in the corner
                continue
            if self.currentLoad(rt1) + self.currentLoad(rt2) > self.capacity: #capacity overloaded
                continue
            
            self.merge_routes(n1, n2) # merge is applicable

        self.sol.cost = self.CalculateTotalCost(self.sol)

    def currentLoad(self,rt):
        td = sum(n.demand for n in rt.sequenceOfNodes) #total demand of rt route
        return td
